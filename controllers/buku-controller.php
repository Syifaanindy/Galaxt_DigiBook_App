...
function prosesTambahBuku() {
    ...
    if (!empty($_FILES['cover_buku']['name'])) {
        ...
        if (move_uploaded_file($_FILES['cover_buku']['tmp_name'], $target_dir . $cover_name)) {
            $cover_image = "assets/uploads/" . $cover_name;
        }
    }
...
function prosesUpdateBuku() {
    ...
    // Mengamankan data file lama jika saat edit form file dikosongkan
    ...
    // Mengirimkan 10 parameter lengkap termasuk ID dan Price untuk update data
